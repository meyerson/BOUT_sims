 ...
 *******************************************************************************/
//#include </opt/apps/python/2.7.1/include/python2.7/Python.h>
#include </share/home/01523/meyerson/local/include/python2.7/Python.h>
...
BoutReal nu_hat, mui_hat, wci, nueix, nuiix, sig_par;
BoutReal beta_p;
...
int physics_run(BoutReal t)
{
  ...
  if(evolve_vi) {
    ...
    ddt(Vi) = lowPass(ddt(Vi),12);
    //Vi = lowPass(Vi,12);
//ddt(Vi) = nl_filter_y(ddt(Vi),4);
  ...
  if(evolve_rho) {
    ...
    */
    ...
    //ddt(rho) += Laplacian(rho);
     ...
     //rho = lowPass(rho,10);
     ddt(rho) = lowPass(ddt(rho),12);
    
     
     //ddt(rho) = nl_filter_y(ddt(rho),4);
...
// int py_try(int argc,std::string argv[])
// {
...
//       if (sys_path == NULL) 
// 	return NULL; 
//       path = PyString_FromString("/share/home/01523/BOUT/tools/pylib/post_bout");
//       if (path == NULL) 
// 	return NULL; 
...
//       pName = PyString_FromString(argv[0].c_str()); //module name
      
//       output.write("pName: %s \n", argv[0].c_str());
   
//       pModule = PyImport_ImportModule(argv[0].c_str());
...
// 	pFunc = PyObject_GetAttrString(pModule,argv[1].c_str()); //single out a function from  a given module
      
// 	output.write("PyCallable_Check(pFunc): %i \n",PyCallable_Check(pFunc));
// 	//pValue = PyInt_FromLong(atoi(argv[2].c_str()));
//       	//output.write("pValue: %i \n",pValue);
	

// 	if (pFunc && PyCallable_Check(pFunc)) {
// 	  //parse the argument to pass to the python function
// 	  // if (argc == 3){
// 	  //   if (argv[2].c_str() == NULL)
// 	  //     pArgs = NULL;
// 	  // }
// 	  if(argc ==2)
// 	    pArgs = NULL;
...
// 	    for (i = 0; i < argc - 2; ++i) {
// 	      //pValue = PyInt_FromLong(atoi(argv[i + 2].c_str()));
// 	      pValue = PyString_FromString(argv[i + 2].c_str());
// 	      //output.write("Args: %s \n",pValue);
...
// 		return 1;
// 	      }
// 	      output.write("arg[%i]: %s \n",i,argv[i + 2].c_str());
// 	      PyTuple_SetItem(pArgs, i, pValue);
// 	    }
// 	  }
// 	  //pValue = PyString_FromString(argv[2].c_str());
