...
def aveplot(pp,data,title='chaotic '+r"$\alpha \pm \sigma$",x_label=r'$\rho_s$',
    ...
    r = x0+np.arange(data.shape[0])*dx
#line, = sm.plot(alpha, color='blue', lw=2)
    sm.plot(r,alpha, lw=2, label=label, color='blue')
    sm.fill_between(r,np.max(data,axis=1), np.min(data,axis=1), facecolor='yellow', alpha=0.5)
    sm.set_title(title)
...
a_frm = Frame(mask,meta={'stationary':True,'dx':dx,'dy':dy,'x0':x0,
                      'xlabel':r'$\rho_s$','fontsz':20})
...
fig = plt.figure()
a_frm = Frame(np.log(a),meta={'stationary':True,'dx':dx,'dy':dy,'x0':x0,
                              'xlabel':r'$\rho_s$','fontsz':20,
